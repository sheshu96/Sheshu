...
def display_records():
    records = connection.execute("SELECT * FROM my_table WHERE Status = 'A'")
...
def update_record():
    ...
    if is_record_found == True:
        ...
        connection.execute('UPDATE my_table SET ' + field_names[int(updatable_fields[choice - 1]) - 1] + ' = ' + "\"" + field_value_to_update_record + "\""+ ' WHERE ' + field_names[0] + ' = ' + id_to_update_record)
        connection.commit()
        print(messages[1])
    else:
        print_record_not_found()
...
column_names = connection.execute("pragma table_info('my_table')")
field_names = []
